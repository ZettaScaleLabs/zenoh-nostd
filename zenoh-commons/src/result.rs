...
/// General errors that can occur in various parts of the library.
#[repr(u8)]
...
macro_rules! zerr {
    (
        ...
        $(
            ...
            #[cfg(feature = "ctx")]
            impl From<$name> for $crate::result::ZCtxError<$name> {
                fn from(value: $name) -> Self {
                    $crate::result::ZCtxError {
                        error: value,
                        ctx: $crate::result::Vec::new(),
                    }
                }
            }

            #[cfg(feature = "ctx")]
            impl From<$name> for $crate::result::ZCtxError<$crate::result::ZError> {
                fn from(value: $name) -> Self {
                    $crate::result::ZCtxError {
                        error: value.into(),
                        ctx: $crate::result::Vec::new(),
            ...
            #[cfg(feature = "ctx")]
            impl From<$crate::result::ZCtxError<$name>> for $crate::result::ZCtxError<$crate::result::ZError> {
                fn from(value: $crate::result::ZCtxError<$name>) -> Self {
                    $crate::result::ZCtxError {
...
pub struct ZCtxError<E> {
    pub error: E,
    pub ctx: Vec<&'static str, 16>,
}

#[cfg(feature = "ctx")]
impl<E> ZCtxError<E> {
    pub fn with_ctx(mut self, ctx: &'static str) -> Self {
        let _ = self.ctx.push(ctx);
        self
    }
}

#[cfg(feature = "ctx")]
impl<E: core::fmt::Display> core::fmt::Display for ZCtxError<E> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        if self.ctx.is_empty() {
            write!(f, "{}", self.error)
        } else {
            if self.ctx.len() == 1 {
                write!(f, "{} at {}", self.error, self.ctx[0])
            } else {
                write!(f, "{} at", self.error)?;
                for ctx in &self.ctx {
                    write!(f, "\n  - {}", ctx)?;
                }
                Ok(())
...
#[cfg(feature = "ctx")]
pub type ZResult<T, E = ZError> = core::result::Result<T, ZCtxError<E>>;
...
macro_rules! zctx {
    ...
    ($res:expr) => {
        $res.map_err(|e| e.with_ctx($crate::zctx!()))
    };
}
...
macro_rules! zbail {
    ($err:expr) => {
        return Err($crate::result::ZCtxError {
            error: $err.into(),
            ctx: $crate::result::Vec::from_slice(&[$crate::zctx!()]).unwrap(),
        })
    };
}
