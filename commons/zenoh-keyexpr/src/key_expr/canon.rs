...
fn canonizer() {
    ...
    OwnedKeyExpr::autocanonize(String::<16>::from_str("/a/b/").unwrap()).unwrap_err();
    OwnedKeyExpr::autocanonize(String::<16>::from_str("/a/b").unwrap()).unwrap_err();
    OwnedKeyExpr::autocanonize(String::<16>::from_str("a/b/").unwrap()).unwrap_err();
    OwnedKeyExpr::autocanonize(String::<16>::from_str("a/b/*$*").unwrap()).unwrap_err();
    OwnedKeyExpr::autocanonize(String::<16>::from_str("a/b/$**").unwrap()).unwrap_err();
    ...
    let mut s = String::<32>::from_str("hello/foo$*$*/bar").unwrap();
    s.canonize();
    assert_eq!(s, "hello/foo$*/bar");

    // Any contiguous sequence of ** chunks is replaced by a single ** chunk
    let mut s = String::<32>::from_str("hello/**/**/bye").unwrap();
    s.canonize();
    assert_eq!(s, "hello/**/bye");
    let mut s = String::<32>::from_str("hello/**/**").unwrap();
    s.canonize();
    assert_eq!(s, "hello/**");

    // Any $* chunk is replaced by a * chunk
    let mut s = String::<16>::from_str("hello/$*/bye").unwrap();
    s.canonize();
    assert_eq!(s, "hello/*/bye");
    let mut s = String::<16>::from_str("hello/$*$*/bye").unwrap();
    s.canonize();
    ...
    // **/* is replaced by */**
    let mut s = String::<16>::from_str("hello/**/*").unwrap();
    s.canonize();
    assert_eq!(s, "hello/*/**");
