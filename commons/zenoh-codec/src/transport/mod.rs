...
//     fn read(self, reader: &mut R) -> ZResult<TransportMessageLowLatency, Self::Error> {
...
impl<R, const L: usize> RCodec<TransportMessage, &mut R> for (Zenoh080, ZSliceLen<L>)
where
    R: Reader + BacktrackableReader,
{
    type Error = ZError;

    fn read(self, reader: &mut R) -> ZResult<TransportMessage> {
        ...
        let body = match imsg::mid(codec.0.header) {
            ...
            id::INIT => {
                if !imsg::has_flag(codec.0.header, zenoh_protocol::transport::init::flag::A) {
                    TransportBody::InitSyn(codec.0.read(&mut *reader)?)
                } else {
                    TransportBody::InitAck(codec.read(&mut *reader)?)
                }
            }
            id::OPEN => {
                if !imsg::has_flag(codec.0.header, zenoh_protocol::transport::open::flag::A) {
                    TransportBody::OpenSyn(codec.read(&mut *reader)?)
                } else {
                    TransportBody::OpenAck(codec.0.read(&mut *reader)?)
                }
            }
    ...
    R: Reader,
{
    ...
    fn read(self, reader: &mut R) -> ZResult<(ext::PatchType<{ ID }>, bool)> {
        let header: u8 = self.read(&mut *reader)?;
        let codec = Zenoh080Header::new(header);
        codec.read(reader)
    }
