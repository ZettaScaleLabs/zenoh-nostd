    ...
    R: Reader,
{
    ...
    fn read(self, reader: &mut R) -> ZResult<PrioritySn, Self::Error> {
    ...
    R: Reader,
{
    ...
    fn read(self, reader: &mut R) -> ZResult<(ext::QoSType, bool), Self::Error> {
    ...
    R: Reader,
{
    ...
    fn read(self, reader: &mut R) -> ZResult<Join, Self::Error> {
        let header: u8 = self.read(&mut *reader)?;
        let codec = Zenoh080Header::new(header);
        codec.read(reader)
    }
