...
pub fn read<'a>(
    ...
    let codec = Zenoh080;
    let (u, has_ext): (ZExtUnknown, bool) = codec.read(reader)?;

    if iext::eid(header) != u.id {
...
// ZExtZBuf
impl<'a, const ID: u8> WCodec<'a, (&ZExtZBuf<'a, ID>, bool)> for Zenoh080 {
    fn write(
        &self,
        message: (&ZExtZBuf<'a, { ID }>, bool),
        writer: &mut ZBufWriter<'a>,
    ) -> ZResult<()> {
        let (x, more) = message;
...
// ZExtUnknown
impl<'a> WCodec<'a, (&ZExtUnknown<'a>, bool)> for Zenoh080 {
    fn write(&self, message: (&ZExtUnknown<'a>, bool), writer: &mut ZBufWriter<'a>) -> ZResult<()> {
        let (x, more) = message;
...
// &[ZExtUnknown]
impl<'a> WCodec<'a, &'_ [ZExtUnknown<'a>]> for Zenoh080 {
    fn write(&self, message: &'_ [ZExtUnknown<'a>], writer: &mut ZBufWriter<'a>) -> ZResult<()> {
        let len = message.len();
