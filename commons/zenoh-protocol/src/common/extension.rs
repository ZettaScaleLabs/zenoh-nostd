...
use zenoh_result::{zerr, ZError, ZE};
...
impl<const ID: u8, const N: usize, const L: usize> TryFrom<ZExtUnknown<N, L>> for ZExtUnit<{ ID }> {
    type Error = ZError;

    fn try_from(v: ZExtUnknown<N, L>) -> Result<Self, Self::Error> {
        if v.id != ID {
            return Err(zerr!(ZE::DidntConvert));
        }
        match v.body {
            ZExtBody::Unit => Ok(Self::new()),
            _ => Err(zerr!(ZE::DidntConvert)),
        }
    }
}
...
impl<const ID: u8, const N: usize, const L: usize> TryFrom<ZExtUnknown<N, L>> for ZExtZ64<{ ID }> {
    type Error = ZError;
...
pub struct ZExtZBuf<const ID: u8, const N: usize, const L: usize> {
    pub value: ZBuf<N, L>,
}

impl<const ID: u8, const N: usize, const L: usize> ZExtZBuf<{ ID }, N, L> {
    pub const ID: u8 = ID;

    pub const fn new(value: ZBuf<N, L>) -> Self {
        Self { value }
    }
    ...
    pub fn transmute<const DI: u8>(self) -> ZExtZBuf<{ DI }, N, L> {
        ZExtZBuf::new(self.value)
    }
}

impl<const ID: u8, const N: usize, const L: usize> Debug for ZExtZBuf<{ ID }, N, L> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl<const ID: u8, const N: usize, const L: usize> TryFrom<ZExtUnknown<N, L>>
    for ZExtZBuf<{ ID }, N, L>
{
    type Error = ZError;
...
pub enum ZExtBody<const N: usize, const L: usize> {
    #[default]
    Unit,
    Z64(u64),
    ZBuf(ZBuf<N, L>),
...
#[derive(Clone, PartialEq, Eq)]
pub struct ZExtUnknown<const N: usize, const L: usize> {
    pub id: u8,
    pub body: ZExtBody<N, L>,
}

impl<const N: usize, const L: usize> ZExtUnknown<N, L> {
    pub const fn new(id: u8, mandatory: bool, body: ZExtBody<N, L>) -> Self {
        let enc = match &body {
...
impl<const N: usize, const L: usize> Debug for ZExtUnknown<N, L> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl<const ID: u8, const N: usize, const L: usize> From<ZExtUnit<{ ID }>> for ZExtUnknown<N, L> {
...
impl<const ID: u8, const N: usize, const L: usize> From<ZExtZ64<{ ID }>> for ZExtUnknown<N, L> {
...
impl<const ID: u8, const N: usize, const L: usize> From<ZExtZBuf<{ ID }, N, L>>
    for ZExtUnknown<N, L>
{
    fn from(e: ZExtZBuf<{ ID }, N, L>) -> Self {
        ZExtUnknown {
            id: ID,
